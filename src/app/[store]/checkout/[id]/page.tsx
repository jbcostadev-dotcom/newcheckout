...
import { CheckoutData, CheckoutProcessResponse } from "@/types";
...
export default function CheckoutPage() {
    ...
    const fetchCheckout = async () => {
      try {
        ...
        const res = await apiGet<CheckoutData>(
          `/checkout?domain=${encodeURIComponent(domain)}&product_id=${productId}`
        );
        setData(res);
      } catch (err) {
  ...
  const handlePayment = async () => {
    ...
    try {
      const domain = getStoreIdentifier();
      const res = await apiPost<CheckoutProcessResponse>(
        "/checkout/process",
        {
          domain,
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#0a0a1a] p-4 text-center text-white">
        <div>
          ...
          <p className="mt-2 text-sm opacity-60">{error ?? "Produto não encontrado."}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex min-h-screen flex-col" style={{ background: bgBase, color: textColor }}>
      <header
        className="flex items-center gap-3 px-6 py-4"
        style={{ background: primary, color: "#fff" }}
      >
        {settings.logo_url && (
          <img src={settings.logo_url} alt="" className="h-8 rounded object-contain" />
        )}
      ...
      {settings.banner_url && (
        <img src={settings.banner_url} alt="" className="w-full object-cover" style={{ maxHeight: 200 }} />
      )}

      {orderPaid ? (
        <div className="flex flex-1 items-center justify-center p-8">
          <div className="rounded-xl p-10 text-center" style={{ background: cardBg, border: `1px solid ${borderColor}` }}>
            <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full" style={{ background: primary }}>
        ...
        <main className="mx-auto flex w-full max-w-5xl flex-1 gap-8 p-8 md:flex-row flex-col">
          <div className="flex-1">
            <div className="rounded-xl p-8" style={{ background: cardBg, border: `1px solid ${borderColor}` }}>
              <h2 className="mb-6 text-lg font-bold">1. Dados de Contato</h2>
              <div className="space-y-3">
                <input type="text" placeholder="Nome Completo *" value={customerName} onChange={(e) => setCustomerName(e.target.value)} style={inputStyle} />
                <input type="email" placeholder="E-mail *" value={customerEmail} onChange={(e) => setCustomerEmail(e.target.value)} style={inputStyle} />
                <input type="tel" placeholder="WhatsApp" value={customerPhone} onChange={(e) => setCustomerPhone(e.target.value)} style={inputStyle} />
                <input type="text" placeholder="CPF (opcional)" value={customerDocument} onChange={(e) => setCustomerDocument(e.target.value)} style={inputStyle} />
              </div>

              <h2 className="mb-4 mt-8 text-lg font-bold">2. Pagamento</h2>
              <div className="flex gap-3">
                <button
                  onClick={() => { setPaymentMethod("pix"); setPixQrCode(null); setPixCopiaCola(null); }}
                  style={{
                    flex: 1, padding: "14px", borderRadius: 8,
                    border: `2px solid ${paymentMethod === "pix" ? primary : borderColor}`,
                ...
                <button
                  onClick={() => { setPaymentMethod("credit_card"); setPixQrCode(null); setPixCopiaCola(null); }}
                  style={{
                    flex: 1, padding: "14px", borderRadius: 8,
                    border: `2px solid ${paymentMethod === "credit_card" ? primary : borderColor}`,
                    background: paymentMethod === "credit_card" ? primary : "transparent",
              ...
              {pixQrCode && (
                <div className="mt-6 rounded-lg p-4" style={{ background: inputBg }}>
                  ...
                  <img src={pixQrCode} alt="QR Code PIX" className="mx-auto mb-3 h-48 w-48 rounded-lg" />
                  {pixCopiaCola && (
              ...
              <button
                ...
                style={{
                  width: "100%", marginTop: 24, padding: "16px", borderRadius: 8,
              ...
              >
                {processing ? "Processando..." : pixQrCode ? "Aguardando pagamento..." : (settings.button_text || "Finalizar Compra")}
              </button>
            </div>
          </div>
            ...
            <div className="rounded-xl p-8" style={{ background: cardBg, border: `1px solid ${borderColor}` }}>
              ...
              <div className="flex items-center gap-4 pb-4" style={{ borderBottom: `1px solid ${borderColor}` }}>
                {product.image_url ? (
                  ...
                  <div className="flex h-16 w-16 items-center justify-center rounded-lg" style={{ background: inputBg }}>
                    <svg className="h-6 w-6" style={{ color: mutedText }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                ...
                <div>
                  ...
                  <p style={{ color: mutedText }}>{formatCurrency(Number(product.price))}</p>
                </div>
              </div>
              ...
              <div className="mt-4 flex justify-between text-lg font-bold" style={{ borderTop: `1px solid ${borderColor}`, paddingTop: 16 }}>
                <span>Total</span>
                <span style={{ color: primary }}>{formatCurrency(Number(product.price))}</span>
              </div>
            </div>
          </div>
        </main>
      ...
      <footer className="px-6 py-3 text-center text-xs" style={{ color: mutedText, background: cardBg }}>
